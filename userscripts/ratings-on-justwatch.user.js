...
(() => {
  ...
  //* GM_config
  GM_config.init({
    id: 'ratings-on-justwatch',
    title: `Ratings on JustWatch v${GM.info.script.version} Settings`,
    fields: {
      OMDbApiKey: {
        label: 'OMDb API Key',
    ...
    events: {
      ...
      save: () => {
        if (!GM_config.isOpen && GM_config.get('OMDbApiKey') === '') {
          window.alert('Ratings on JustWatch: check your settings and save')
        } else {
          window.alert('Ratings on JustWatch: settings saved')
          GM_config.close()
