...
(async () => {
  ...
  const color = 'rgb(50, 25, 25)'
  const colorText = 'rgb(232, 230, 227)'
  const colorLink = 'rgb(255, 132, 132)'
  ...
  const hideNonLatinScripts = (element) => {
    ...
    if (nonLatins.test(name) || nonLatins.test(description)) {
      $(element).addClass('non-latin').css('background-color', color).css('color', colorText).hide().find('a').css('color', colorLink)
    }
  }
  ...
  const hideBlacklistedScripts = (element) => {
    ...
    if (blacklist.test(name) || blacklist.test(description)) {
      $(element).addClass('blacklisted').css('background-color', color).css('color', colorText).hide().find('a').css('color', colorLink)
    }
  }
