...
(() => {
  ...
  const hideScript = async (element, list) => {
    ...
    if (name in JSON.parse(await GM.getValue('hiddenList', '{}')) && list) {
      $(element).addClass('hidden-script').css('background-color', color).hide()
    } else if (name in JSON.parse(await GM.getValue('hiddenList', '{}')) && !list) {
      $(element).addClass('hidden-script').find('.col-sm-8, .col-sm-8 .panel-body').css('background-color', color)
    }
    ...
    $(element).find('.block-button').click(async (event) => {
      ...
      if (!(name in hiddenList)) {
        ...
        if (list) {
          $(element).hide(750).addClass('hidden-script').css('background-color', color)
            .find('.block-button').text(blockLabel($(element).hasClass('hidden-script')))
        } else {
          $(element).addClass('hidden-script')
            .find('.col-sm-8, .col-sm-8 .panel-body').css('background-color', color)
            .find('.block-button').text(blockLabel($(element).hasClass('hidden-script')))
        }
      } else { // ...else remove it
