...
this.Ratings = (function () {
  ...
  class Ratings {
    ...
    _tomatoScore = score => score < 60 ? 'Rotten' : 'Fresh'
    _color = metascore => metascore < 40 ? '#ff0000' : (metascore >= 40 && metascore <= 60 ? '#ffcc33' : '#66cc33')
    async get ({ id }) {
      ...
      const tomato = await this._tomato.search({
        ...
        switch (omdb.Type) {
          case 'movie':
            return data.movies.map(item => item).find(item => this._n(item.name) === this._n(omdb.Title) && item.year === Number.parseInt(omdb.Year))

          case 'series':
            return data.tvSeries.map(item => item).find(item => this._n(item.title) === this._n(omdb.Title) && item.startYear === Number.parseInt(/\d{4}/.exec(omdb.Year)[0]))
        }
