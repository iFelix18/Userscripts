...
(() => {
  ...
  this.Ratings = class {
    ...
    constructor (config = {}) {
       ...
       */
      this._config = {
        omdb_apikey: config.omdb_apikey,
       ...
       */
      this._omdb = new OMDb({
        apikey: this._config.omdb_apikey,
        url: this._config.omdb_url,
      ...
      /**
       * Return formatted IMDb votes
       * @param {number}          votes IMDb votes
       * @returns {string|number}
       */
      this._imdbVotes = (votes) => {
      ...
      try {
        return await new Promise((resolve, reject) => {
          if (cache !== undefined && ((Date.now() - cache.time) < this._config.cache_period) && !this._config.debug) { // cache valid
            const data = { omdb: cache.data.omdb, tomato: cache.data.tomato } // cached data
            ...
            this._omdb.get({
              ...
              this._tomato.search({
                ...
                const data = { omdb: omdbData, tomato: tomatoData } // APIs data
      ...
      try {
        return await new Promise((resolve, reject) => {
          const imdb = {
            ...
            votes: data.omdb && data.omdb.imdbVotes && data.omdb.imdbVotes !== 'N/A' ? this._imdbVotes(Number(data.omdb.imdbVotes.replace(/,/g, ''))) : 'N/A'
          }
          const tomatometer = {
          ...
          const ratings = { imdb, tomatometer, metascore }
          this._debug(ratings)
          resolve(ratings)
        })
      } catch (error) {
